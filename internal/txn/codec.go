...
const recordEncodingVersion byte = 1
...
func (r Record) MarshalBinary() ([]byte, error) {
	...
	buf := make([]byte, 0, 1+16+1+(5*hlc.EncodedSize)+(4*4)+(binary.MaxVarintLen64*(1+len(r.TouchedRanges))))
...
func (r *Record) UnmarshalBinary(data []byte) error {
	...
	if data[0] != recordEncodingVersion {
